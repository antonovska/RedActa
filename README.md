...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/antonovska/RedActa/blob/master/notebooks/demo.ipynb)
...
```powershell
python -m redacta.cli --help
```

Запустить один кейс из workspace, где есть `redactions/<case-id>/`:

```powershell
python -m redacta.cli run-case --case-id 28 --workspace-root . --models-config config/models.example.json --output-json artifacts/28/result.json
```

Запустить несколько кейсов:

```powershell
python -m redacta.cli run-batch --case-id 13 --case-id 14 --workspace-root . --models-config config/models.example.json --output-json artifacts/batch.json
```
...
6. Запускает пайплайн через `redacta.colab_runner.run_uploaded_pair`.
